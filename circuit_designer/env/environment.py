...
class CircuitDesigner(gym.Env):
  """ Quantum Circuit Environment: build a quantum circuit gate-by-gate for a desired challenge.

  Attributes
  qubits [int]: number of available qubits for quantum circuit
  depth [int]: maximum depth desired for quantum circuit
  challenge [str]: RL challenge for which the circuit is to be built (see Reward class)
  punish [bool]: specifies whether depth of circuit should be punished
  device [qml.device]: quantum device to use (see PennyLane)

  action_space [gymnasium.spaces]: action space consisting of (gate: Box (int), target qubit: Box (int), params: Box (float))
  observation_space [gymnasium.spaces]: complex observation of state in the computational basis as a Box with [real, imag]
