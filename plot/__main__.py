...
parser.add_argument('-r', dest='random_baseline', action='store_true')
...
parser.add_argument('-m', dest='metrics', nargs='+', default=['Return', 'Qubits', 'Depth'], choices=options.keys(), help='Experiment keys to group plotted data by.')
...
groupby = args.pop('groupby'); mergeon = args.pop('mergeon'); random_baseline = args.pop('random_baseline')
...
if random_baseline: [fetch_random(args['base'], exp) for exp in experiments]
